...
get_interval_by_sec(stopwatch *sw){
	if (sw == NULL)
		return 0;
	return ((float)(sw->end.tv_sec-sw->begin.tv_sec)+(float)(sw->end.tv_usec-sw->begin.tv_usec)/1000000);
}
...
func_ret_t
lud_write(float *m, float *lu, int matrix_dim){
	int i,j,k;
	...
	write_matrix(tmp,matrix_dim,"res.dat");
...
lud_verify(float *m, float *lu, int matrix_dim){
	...
	for (i=0; i<matrix_dim; i++){
		for (j=0; j<matrix_dim; j++){
			if ( fabs(m[i*matrix_dim+j]-tmp[i*matrix_dim+j]) > 0.0001) {
				printf("dismatch at (%d, %d): (o)%.10f (n)%.10f\n", i, j, m[i*matrix_dim+j], tmp[i*matrix_dim+j]);
				mismatches++;
			}
		}
	}
...
void write_matrix(float* m, int matrix_dim,char* file)
{
//	char output_file[32] ="matrix_lu.dat";
	...
	if(fp!=NULL){
		...
		for(int i=0; i<matrix_dim; i++){
			for(int j=0; j<matrix_dim; j++)//cols and rows are == to num_matrix
