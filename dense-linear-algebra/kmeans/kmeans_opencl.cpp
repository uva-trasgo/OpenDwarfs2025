...
#define THREADS_PER_DIM 16 //modifica el tamaño de local y global work groups
...
/* allocate device memory, calculate number of blocks and threads, and invert the data array */
	extern "C"
void allocateMemory(int npoints, int nfeatures, int nclusters, float **features,int blockSize)
{	
	cl_int errcode;
	...
	if(blockSize > 0 && blockSize<=globalWorkSize){
		localWorkSize = blockSize;
		localSizeBlock = blockSize;
	}else
		localWorkSize = num_threads;
	
	//printf("allocMem\nlocalWorkSize[0] = %zu globalWorkSize[0] = %zu\n\
	  ...
	  )
{
	...
	//printf("2D config\nlocalWorkSize[0] = %zu globalWorkSize[0] = %zu\n\
	localWorkSize[1] = %zu globalWorkSize[1] = %zu\n\n",localWorkSize[0],
	//globalWorkSize[0], localWorkSize[1],globalWorkSize[1]);
