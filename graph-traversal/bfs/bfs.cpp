...
/**Reads the file length from which to make calculations**/
...
void BFSGraph(int argc, char ** argv)
{
	...
	if (argc >= 5 && (strcmp(argv[3],"--localWorkSize") == 0 || strcmp(argv[3],"-L") == 0) ){
		...
		if(localWork >0 && localWork<no_of_nodes){
			printf("We're gonna use %zu for local workSize\n",localWork);
		}else{
				printf("Invalid size of localWorkSize");
				helpMenu();
				exit(-1);
		}
	}
	...
	if(localWork == 0){
		printf("Size of local work not set. The system will determine it by itself\n");
		localWork = no_of_nodes>256 ? 256: no_of_nodes;
	}
	...
	{
		fscanf(fp, "%d", &id);
		fscanf(fp, "%d", &cost);//never used in the system.
		h_graph_edges[i] = id;
	}
	...
	//Check if user has set a certain number of threads
	maxThreads[0] = no_of_nodes>256 ? 256 : no_of_nodes; 
	size_t WorkSize[1] = {(no_of_nodes/localWork)*localWork + ((no_of_nodes%localWork)==0?0:localWork)}; // one dimensional Range
	size_t localWorkSize[1]= {localWork>0 ? localWork : maxThreads[0]};
