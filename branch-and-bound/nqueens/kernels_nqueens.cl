...
//What header of the func is being used
#ifdef USE_ATOMICS
__kernel __attribute__((reqd_work_group_size(WORK_ITEMS, 1,1))) void nqueen(int board_size, int level, int threads, int pitch, __global uint* params, __global uint* results,
																			__constant uint* forbidden_data, __global int* global_index)
#else
__kernel __attribute__((reqd_work_group_size(WORK_ITEMS, 1,1))) void nqueen(int board_size, int level, int threads, int pitch, __global uint* params, __global uint* results, 
																			__constant uint* forbidden_data)
#endif
{
	int idx = get_global_id(0);
	int tid = get_local_id(0);
