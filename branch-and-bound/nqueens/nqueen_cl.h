...
#define CL_TARGET_OPENCL_VERSION 110//ESPECIFICA LA VERSION DE OPENCL A USAR
#include <CL/cl.h>
...
class NQueenSolver
{
	struct solver_info
	{
		solver_info() : m_Queue(0), m_Program(0), m_NQueen(0), m_NQueen1(0), 
						m_ParamBuffer(0), m_ResultBuffer(0), m_ForbiddenBuffer(0), 
						m_GlobalIndex(0), m_Event(0),m_bCPU(false), m_bEnableAtomics(false),
						m_bEnableVectorize(false), m_bEnableChar(false), m_bEnableLocal(false),
						m_nMaxWorkItems(0), m_nThreads(0)
		{
		}
...
public:
	...
	NQueenSolver(cl_context context, std::vector<cl_device_id> devices, bool profiling = false, 
				 int threads = 0, int block_size = 0, bool force_local = false, 
				 bool force_no_atomics = false, bool force_no_vec = false, 
				 bool force_vec4 = false);
	~NQueenSolver();
