...
NQueenSolver::NQueenSolver(	cl_context context,
						   	std::vector<cl_device_id> devices, 
						   	bool profiling, int threads, 
						   	int block_size, bool force_local, 
						   	bool force_no_atomics, bool force_no_vec, 
						   	bool force_vec4) :
							m_Context(context), m_bProfiling(profiling),
							m_bForceLocal(force_local), m_bForceNoAtomics(force_no_atomics),
							m_bForceNoVectorization(force_no_vec), m_bForceVec4(force_vec4)
{
	cl_int err;
...
//Do we really need these function for a simple if clause??
...
//Get some device data to build the program for OpenCL
//Limits some of the parameter for execution independently of device type
void NQueenSolver::InitKernels(int i, int block_size)
{
	...
	if(!m_bForceNoVectorization && !m_SolverInfo[i].m_bCPU && vector_width != 1) {
		...
		m_SolverInfo[i].m_bEnableLocal = true; //Never used ??? 
	}
...
//With some parameters given, build and config the OpenCL kernel of the program
void NQueenSolver::BuildProgram(int i, const std::string& program, int vector_width, int work_items)
{
	...
	//Setings for using OpenCL nqueens, stored in a var
    ...
    //Sets the kernel with the settings
...
//Many binary comparatios for what??
...
long long NQueenSolver::Compute(int board_size, long long* unique)
{
	...
	//Never get to more than level 11.

	long long total = 10000000000LL;
	//total=total/10
	total /= 10;	// for atomics
	int level = 0;
	int i = board_size;//value of N stored, not N*N
	...
	//No point in this if it always ginna be 1 ???
	...
	//max_pitch =max_threads;//Why not make it like this
	max_pitch = (max_threads + 15) & ~0xf;//insetad of making a binary AND like this
	...
	for(int i = 0; i < threads.size(); i++) { //Will be always 1 in OpenDwarfs
		...
		m_SolverInfo[i].m_ParamBuffer = clCreateBuffer(m_Context, CL_MEM_READ_ONLY, max_pitch * sizeof(int) * 64, 0, &err);
		CHECK_ERROR(err);
	...
	for(int j = 0; j < board_size / 2; j++) {
		...
		//make some shifts (R/L) to variables
		masks[0] = (1 << j); 
		left_masks[0] = 1 << (j + 1); 
		right_masks[0] = (1 << j) >> 1;; 
		...
		while(i >= 0) {
			if(j == 0) {
				if(i >= 1) {
					...
					ns[i + 1] = (m + 1) & ~m;
					//~m: means that is giving to m 1 to its most valuable bit
				}
				else {
					ns[i + 1] = ((ms[i] + 1) & ~ms[i]);
					if(i == 0) {
			...
			if(i == board_size - level - 1) {
				...
				if(total_size == max_threads) {
					...
					//writting in forbidden buffer. Why though??
					...
					err = clEnqueueWriteBuffer(	m_SolverInfo[device_idx].m_Queue, m_SolverInfo[device_idx].m_ParamBuffer, 
												CL_FALSE, 0, max_pitch * sizeof(int) * (4 + 32), &mask_vector[0], 0, 0, &ocdTempEvent );
					clFinish(m_SolverInfo[device_idx].m_Queue);
