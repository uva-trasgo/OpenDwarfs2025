...
int main(int argc, char** argv)
{
	...
	bool profiling = false;//never changed 
	int threads = 0;
	int block_size = 0;
	...
	//Makes sure Block_size can divide Threads 1 to 1, in case threads is only set
	if (block_size == 0 && threads != 0)
	{
		block_size = threads;
	}
	...
	if (block_size != 0 && threads != 0){
		if(block_size>threads){
			printf("Error Blocksize cant be bigger than Threads\n");
			exit(-1);
		}
