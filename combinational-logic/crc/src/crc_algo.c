...
void fPrintTimeDiff(FILE* file,struct timeval start, struct timeval end)
{
	fprintf(file,"\n\n Time Duration = %ld microseconds\n", ((end.tv_sec * 1000000 + end.tv_usec)
				- (start.tv_sec * 1000000 + start.tv_usec)));
}
				...
				for(ii=0; ii<num_execs; ii++)
				{
					if(verbosity) printf("Beginning execution #%u of %u...\n",ii+1,num_execs);
						...
						for(i=0; i<num_blocks; i++)
						{
							...
							enqueueCRCDevice(&h_num[i*num_parallel_crcs[h]*num_words],&ocl_remainders[i*num_parallel_crcs[h]],global_size,local_size,
								dev_input[i],dev_output[i],&write_page[i],&kernel_exec[i],&read_page[i]);
						}
					...
					clFinish(read_queue);
						...
						if(run_serial) // verify that we have the correct answer with regular C
						{
							...
							FILE* fp_out = fopen("crc_out.txt","w");
							check(fp_out != NULL,"crc_formats.write_crc() - Cannot Open File");	
							gettimeofday(&start,NULL);
							fprintf(fp_out,"    OCL 	  CPU\n");
							for(i=0; i<num_pages; i++)
							{
								...
								fprintf(fp_out,"  %X 	%X\n",cpu_remainder,ocl_remainders[i]);
							}
							gettimeofday(&end,NULL);
							printf("CPU Slice-by-8 CRC Time: ");
							printTimeDiff(start,end);
							fPrintTimeDiff(fp_out,start,end);
							fclose(fp_out);
						}
				}
			}
