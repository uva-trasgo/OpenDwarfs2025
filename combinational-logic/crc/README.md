...
You can create test files as desired by executing the createcrc program (source 
provided under ./src_test/ subfolder.
...
The `createcrc` utility generates random input datasets (messages/pages) for the CRC benchmark.

**Usage:** `./createcrc [OPTIONS]`

* `-n <num>` : Number of pages to generate (Default: 1).
* `-s <size>`: Number of bytes per page (Default: 1024).
* `-f <path>`: Custom output file path (Default: `../test/combinational-logic/crc/crcfile_...`).
* `-p`       : Print generated data to standard output.
* `-R`       : Do not seed the random number generator (produces repeatable results).
* `-S`       : Do not save the generated messages to a file.

**Example:** Generate 10 pages of 4096 bytes each:
`./createcrc -n 10 -s 4096`
