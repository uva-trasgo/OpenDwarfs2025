...
int main(int argc, char** argv)
{
	...
	//May 28 2024
	//Made do_print function, previosly not working
	if (do_print){
		
		for (int i=0; i<num_pages;i++){
			printf("Page == %d/%u; Page_Size == %u \n",i+1,num_pages,page_size);
			
			//Pages are expresed in number of bits
			//1 page == 4 bits
	...
	if(do_save)
	{
		if(!file_path)
		{
		    ...
		    if ( stat("../test/combinational-logic/crc", &sb) != 0 || !S_ISDIR(sb.st_mode) )
		    {
		        printf("Directory no found. Creating crc/ in test/combinational-logic/\n");
		        mkdir("../test/combinational-logic/", 0700)	;
		        mkdir("../test/combinational-logic/crc/", 0700)	;
		    }
