...
int main(int argc, char ** argv)
{
	...
	int blockNum = 14;
	unsigned long int maxElemNum = (unsigned long int)(MAX_LEN + 1) * (MAX_LEN + 1);
    ...
    posix_memalign((void **) &allSequences, AOCL_ALIGNMENT, sizeof(char *)*maxElemNum);//Bigger variable in sizeof should work but, for causion, it is done like this
	...
	seq1D = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_char) * maxElemNum, 0, &err);
	CHKERR(err, "Create seq1D memory");
	seq2D = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_char) * maxElemNum, 0, &err);
	CHKERR(err, "Create seq2D memory");

	//read query sequence
	querySize = readQuerySequence(queryFilePathName, querySequence);
	if (querySize <= 0 || querySize > maxElemNum)
	{
		printf("Query size %ld is out of range (0, %d)\n",
				maxElemNum,
				querySize);
		return 1;
	}
	...
	outSeq1 = new char[maxElemNum];
	outSeq2 = new char[maxElemNum];
	if (outSeq1 == NULL ||
			outSeq2 == NULL)
	{
	...
	outSeq1D = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_char) * maxElemNum, 0, &err);
	CHKERR(err, "Create outSeq1D memory");
	outSeq2D = clCreateBuffer(context, CL_MEM_READ_WRITE, sizeof(cl_char) * maxElemNum, 0, &err);
    ...
    posix_memalign((void **) &diffPos, AOCL_ALIGNMENT, sizeof(int *)*maxElemNum);
    posix_memalign((void **) &threadNum, AOCL_ALIGNMENT, sizeof(int *)*maxElemNum);
	if (threadNum == NULL ||
			diffPos == NULL)
	{
	...
	threadNumD = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_int) * (maxElemNum), 0, &err); 
	CHKERR(err, "Create threadNumD memory");
	diffPosD = clCreateBuffer(context, CL_MEM_READ_ONLY, sizeof(cl_int) * (maxElemNum), 0, &err);
	...
	for (subSequenceNo = 0; subSequenceNo < subSequenceNum; subSequenceNo++)
	{
		...
		if (subSequenceSize <= 0 || subSequenceSize > maxElemNum)
		{
