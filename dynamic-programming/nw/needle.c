...
{
        fprintf(stderr, "\nUsage: %s\n\n\
        [-d dimensions ]: x and y dimensions\n\
        [-p penalty ]:    penalty(positive integer)\n\
        [-b blocksize ]:  blocksize\n\n\
Example: %s -d 256 -p 1 -b 16\n\n",argv[0],argv[0]);
        exit(1);
}
...
void runTest( int argc, char** argv) 
{
	...
	// And at current stage  max_rows needs to equal max_cols
        if (argc>=5 && strcmp("-d",argv[1]) == 0 && strcmp("-p",argv[3]) == 0 )
        {
                max_rows = atoi(argv[2]);
                max_cols = atoi(argv[2]);
                penalty = atoi(argv[4]);

                if(argc==7 && strcmp(argv[5],"-b") == 0)
                        block = atoi(argv[6]);
                else
                        block = BLOCK_SIZE;
	...
	if(atoi(argv[2])%block!=0){
		fprintf(stderr,"The dimension values must be a multiple of %d\n",block);
		exit(1);
	}
	...
	size_t localWorkSize[2] = {block, 1}; //BLOCK_SIZE work items per work-group in 1D only.
	size_t globalWorkSize[2];
	int block_width = ( max_cols - 1 )/block;
...
//#ifdef TRACE
	...
	FILE* fp_out=fopen("needle_out.txt","w");
	check(fp_out != NULL,"neeedle_output - Cannot Open File");	
	...
	for (i = max_rows - 2,  j = max_rows - 2; i>=0, j>=0;){
		...
		if ( i == max_rows - 2 && j == max_rows - 2 )
			...
			fprintf(fp_out,"%d ", output_itemsets[ i * max_cols + j]); //print the first element
