...
void reduce(__global void *key, 
		    ...
		    int valStartIndex)
{
    /* Count how many intermediate values exist for this key.
     * In the stringmatch example, each emitted value is (int)1,
     * so counting values directly gives the frequency. */
    
    int matchCount = 0;
    
    /* Iterate through all intermediate records for this key */
    for (int i = 0; i < valCount; i++) {
        int offsetIndex = valStartIndex + i;
        
        /* Each value is an int (the count "1" from emitIntermediate) */
        __global int *valData = (__global int*)(interVals + interOffsetSizes[offsetIndex].z);
        matchCount += valData[0];
    }
    
    /* Atomically allocate space in the output buffers */
    int outIndex = atom_inc(psCounts);
    
    /* Write the key to the output keys buffer */
    ...
    /* Write the aggregated value to the output values buffer */
    __global int *outVal = (__global int*)(outputVals + atom_add(psValSizes, sizeof(int)));
    *outVal = matchCount;
    
    /* Record the offset and size metadata for this output */
