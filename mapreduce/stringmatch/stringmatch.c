...
int main ( int argc, char *argv[] )
{
    ...
    {/* Read targets and text from file */
        ...
        for (size_t i = 0; i < targetsSize; i++) {
            if (h_inputKeys[i] == ',') file_targets_count++;
        }
    ...
    if (numLinesEstimate < 1) numLinesEstimate = 1;
    ...
    if (numLines < 1) numLines = 1; /*empty text*/
    ...
    if (targetBucketCapacity < 256) targetBucketCapacity = 256;
    ...
    for (int i = 0; i < num_targets; i++) {
        if (h_outputOffsetSizes[i].w > 0 && h_outputOffsetSizes[i].x < (int)outputKeysBufferSize) {
            if (h_outputOffsetSizes[i].y > maxKeyLen) maxKeyLen = h_outputOffsetSizes[i].y;
        }
    }
    ...
    /* --- Cleanup --- */
    clReleaseMemObject(d_inputDataset); clReleaseMemObject(d_inputKeys);
    clReleaseMemObject(d_inputVals); clReleaseMemObject(d_inputOffsetSizes);
    clReleaseMemObject(d_targetOffsetSizes); clReleaseMemObject(d_targetBucketOffsets);
    clReleaseMemObject(d_targetBucketCapacities); clReleaseMemObject(d_targetMatchCounts);
    clReleaseMemObject(d_interVals); clReleaseMemObject(d_interOffsetSizes);
    clReleaseMemObject(d_psKeySizes); clReleaseMemObject(d_psValSizes); clReleaseMemObject(d_psCounts);
    clReleaseMemObject(d_outputKeys); clReleaseMemObject(d_outputVals); clReleaseMemObject(d_outputOffsetSizes);
    ...
    free(h_inputDataset); free(h_inputKeys); free(h_inputVals);
    free(h_inputOffsetSizes);
    free(h_targetOffsetSizes); free(h_targetBucketOffsets);
    free(h_targetBucketCapacities); free(h_targetMatchCounts);
    free(h_outputKeys); free(h_outputVals); free(h_outputOffsetSizes);
