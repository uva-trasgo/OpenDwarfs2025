...
int main (int argc, char **argv)
{
	...
	if ((argc < 4) || (!verify_initial_args(argc, argv) ))
	{
		cerr<<"Format for command line options is \'gem [molname diel_ext diel_int salinity] <options>\'"<<endl
			...
			<<"\t-localSize  	<localWorkSize>"<<endl;

		return 1;
	}

	//read optionals arguments, if theres any
	 if(argc>5)
	 	read_opt_arg_cmd_line( argc, argv, options, flags,&localWork);
