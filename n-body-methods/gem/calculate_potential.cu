...
__global__ void calc_potential_single_step_dev(int eye, int step_size, partitioned_open_struct_dev *devStructPtr)
{
	...
	atom_vert[threadIdx.x].x=devStructPtr->xyz_vert[eye].x;
	atom_vert[threadIdx.x].y=devStructPtr->xyz_vert[eye].y;
	atom_vert[threadIdx.x].z=devStructPtr->xyz_vert[eye].z;
