...
void output_func_exp(const float* matrix,const unsigned int sz,char* file_path)
{
	FILE* fp = fopen(file_path,"a");

	if(fp)
		for (int j = 0; j < sz; ++j)
			fprintf(fp,"%f ",matrix[j]);
		fprintf(fp,"\n");
	fclose(fp);
}

void output_func_host(float* matrix,const unsigned int sz,char* file_path)
{
	FILE* fp = fopen(file_path,"a");
...
int main(int argc, char** argv)
{
	...
	while ((opt = getopt_long(argc, argv, "::vmw:k:i:par:::", long_options, &option_index)) != -1 )
	{
		switch(opt)
		{
			...
			case 'k'://need to have multiple kernerl files for this to work
				if(optarg != NULL)
					optptr = optarg;
				else
					optptr = argv[optind];
			...
			case 'w'://The argument need to be a power of 2 for the system to execute (1,2,4,8,16....)
				if(optarg != NULL)
					optptr = optarg;
				else
					optptr = argv[optind];
			...
			{						//to gather multiple samples of data
				...
				CHKERR(err, "Failed to create a compute kernel!");
					...
					for(k=0; k<num_matrices; k++)
					{
						...
						/* Enqueue Kernel */
						//printf("GlobalSize == %zu\nLocalSize == %zu\n",global_size,wg_sizes[ii]);
				...
				CHKERR(err,"Failed to release kernel!");
					...
					if(do_affirm)
					{
						...
						for(k=0; k<num_matrices; k++)
						{
							...
							output_func_exp(device_out[k],csr[k].num_rows,"Experimental_CSR.txt");
							output_func_host(host_out,csr[k].num_rows,"Control_CSR.txt");
						}
