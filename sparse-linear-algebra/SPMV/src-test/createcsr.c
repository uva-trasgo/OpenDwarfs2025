...
int main(int argc, char** argv)
{
	...
	if(do_save)
	{
		if(!file_path)
		{
			...
			snprintf(file_path,(sizeof(char)*CSR_NAME_MAX_LENGTH),"../test/sparse-linear-algebra/SPMV/csrmatrix_R%u_N%u_D%u_S%02d_%d-%d-%d-%d-%d",num_matrices,N,density,normal_stddev_rounded,(tm.tm_year-100),tm.tm_mon+1,tm.tm_mday,tm.tm_hour,tm.tm_min);
			free_file=1;
		
			//Chekeo y creacion de dir por defecto, si no existe
			struct stat sb = {0};//instanciación de sb para usar stat ()

		   	if (stat("../test/sparse-linear-algebra/SPMV/", &sb) != 0 || !S_ISDIR(sb.st_mode))
		    {
		        printf("Directory no found. Creating SPMV/ in test/sparse-linear-algebra/\n");
		        mkdir("../test/sparse-linear-algebra/", 0700)	;
		        mkdir("../test/sparse-linear-algebra/SPMV/", 0700)	;
		    }
		}
