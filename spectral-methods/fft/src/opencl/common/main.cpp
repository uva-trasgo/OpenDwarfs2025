...
void RunBenchmark(OptionParser &op,int localSize);
...
// #endif
		...
		op.addOption("localSize", OPT_INT, "64", "specify localSize to use with OpenCL", 'N');
...
// #endif  // PARALLEL
		...
		//A way to read and parse option -localSize 
		//Since the parser is failing to work, we added this manual change
		//int localWokrSize = op.getOptionInt("localSize");
		int localWokrSize = -1;
		int i=1;
		do{
			if( (strcmp("--localSize",argv[i])==0 || strcmp("-N",argv[i])==0 )&& atoi(argv[i+1]) >0){
				localWokrSize = atoi (argv[i+1]);
				break;
			}	
			i++;
		}while(i<argc);
