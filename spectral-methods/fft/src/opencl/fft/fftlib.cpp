...
init(OptionParser& op, bool _do_dp, int fftn)
{
	...
	sprintf(opt,"-D FFT_%d -D fftn1=%d -D pow1=%d", fftn, fftn, fftn/16 );
	args += opt;
	err = clBuildProgram(fftProg, 0, NULL, args.c_str(), NULL, NULL);
#ifdef ENABLE_DEBUG
	{
	...
	void 
forward(void* workp, void *temp, int n_ffts, int fftn, int locSZ)
{
	cl_int err;
	size_t localsz0;
	size_t globalsz0;
	...
	//Quick change to allow user to intputs its desired local Size
	if(locSZ>0)
		localsz =localsz0 = locSZ;
	...
	if(fftn> 2048){
			...
			err = clEnqueueNDRangeKernel(commands, fftKrnl1, 1, NULL, 
					...
					NULL, &ocdTempEvent);
				CHKERR(err, "Failed to enqueue kernel fft1D");
