...
template <class T2> void dump1D(OptionParser& op,int localSize);
template <class T2> void dump2D(OptionParser& op,int localSize);


	void
RunBenchmark(OptionParser &op,int localSize)
{
	...
	if(op.getOptionBool("2D"))
		dump2D<cplxflt>(op,localSize);
	else
		dump1D<cplxflt>(op,localSize);
...
// ****************************************************************************
	template <class T2> 
void dump2D(OptionParser& op,int localSize)
{	
	int i;
	void* work, *temp;
	T2* source, * result;
	...
	template <class T2> 
void dump1D(OptionParser& op,int localSize)
{	
	int i;
	int fftn;
	void* work, *temp;
	...
	forward(work, temp, n_ffts, fftn,localSize);

	copyFromDevice(result, work, used_bytes);

	FILE* fOut = fopen("fft_out.txt","w");
	if(fOut!=NULL){
		for (i = 0; i < N; i++) {
			fprintf(fOut, "data[%d] (%g, %g)\n", i, result[i].x, result[i].y);
		}
		printf("Output written to fft_out.txt\n");
	}
	freeDeviceBuffer(work);
	freeDeviceBuffer(temp);
	freeHostBuffer(source);
