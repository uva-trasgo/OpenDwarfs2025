		...
		int length)                   /* number of observations */    
{
	...
	unsigned int index_Thread = (idy * nstates) + idx;
	unsigned int index_Thread2 = ((length-1) * nstates) + idy;

	if (idx < nstates && idy < nstates)
	{
		a_d[index_Thread] =xi_sum_d[index_Thread] / 
			( gamma_sum_d[idy] - alpha_d[index_Thread2] * beta_d[index_Thread2] / sum_ab );
	}
}
