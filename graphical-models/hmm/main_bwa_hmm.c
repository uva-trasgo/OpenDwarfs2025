...
size_t LoadProgramSource(const char *filename, const char **progSrc) 
{
	...
	*progSrc = (const char *) calloc(sizeof(char),len);
	rewind(f);
...
struct timeval toc(struct timeval *timer)
{
...
int estimate_a()
{
	...
	// printf("a_d == alpha_d == %f beta_d == %f xi_sum_d == %f gamma_sum_d == %f sum_ab == %f nstates == %d length == %d\n",
	//   a_d,alpha_d,beta_d,xi_sum_d,gamma_sum_d,sum_ab,nstates,length);
...
//Write output of run_hmm_bwa to console and file	
void write_output(FILE* fp_out,int obs,struct timeval res_time, float log_lik,float* a,float* b,float* pi){
	printf("\n\n\
Observations\t  Time (s)\tLog_likelihood\n \
    %i\t \
    \t %ld.%06ld\t \
    %f\n\n \
a_d\t\tb_d\t\tpi_d\n\
%f\t%f\t%f\n\n",
	obs, res_time.tv_sec, res_time.tv_usec, log_lik,*a,*b,*pi);
		
	//escritura a  fichero
	fprintf(fp_out,"Observations == %i\n",obs);
	fprintf(fp_out,"Time == %ld.%06ld\n", res_time.tv_sec, res_time.tv_usec);
	fprintf(fp_out,"Log_Likehood == %f\n",log_lik);
	fprintf(fp_out,"a_d == %f\nb_d == %f\npi_d == %f\n\n",*a,*b,*pi);
...
static struct option size_opts[] = 
{
	...
	{"max threads per block", 1, NULL, 'T'},
	{0, 0, 0, 0}	
};
...
int main(int argc, char *argv[]) 
{
	...
	int opt, opt_index = 0;
	char v_model;
	while((opt = getopt_long(argc, argv, "::n:s:t:v:T:h:", size_opts, &opt_index)) != -1)
	{
		//printf("here");
		switch(opt)
		{
			...
			case 't':
				...
				break;
			case 'T':
				MAX_THREADS_PER_BLOCK = atoi(optarg)>0 ? atoi(optarg) : MAX_THREADS_PER_BLOCK;
				break;
			case 'h':
				fprintf(stderr, "Usage [input]:%s\n\
		[-n state number | -s symbol number | -t observation number] \n\
		[-v varying model] \n\
	...
	FILE* fp_out = fopen("bwa_hmm_out.txt","w");
	check(fp_out != NULL,"bwa_hmm_out - Cannot Open File");	
 	...
 	if(v_model == 'n')
	{
		...
		write_output(fp_out,n,res_time,log_lik,a,b,pi);
	...
	{	
		...
		write_output(fp_out,s,res_time,log_lik,a,b,pi);
	...
	} else if(v_model == 't')
	{
		...
		write_output(fp_out,t,res_time,log_lik,a,b,pi);
